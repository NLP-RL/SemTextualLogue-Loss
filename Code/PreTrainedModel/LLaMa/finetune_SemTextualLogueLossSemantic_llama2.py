...
dataset_type = "MULTIWoz_finetune_SemTextualLogueSemanticLoss_Train_CLM"
SAVE_MODEL_NAME = "llama2-finetuned-Sem-Textual-Logue-Semantic-Loss-Multiwoz-model"
...
# SemTextualLogueSemantic Model with LLM 
class Sem_Textual_Logue_Semantic_Model(torch.nn.Module):
    ...
    def forward( self, input_ids, attention_mask=None, position_ids=None, past_key_values=None,
      ...
      if labels is not None:
        ...
        semantic_similarity_scores = torch.stack([ torch.tensor( self.cosine_similarity(model_output, gold), dtype=torch.float16 ).to(self.device) for model_output, gold in zip(model_output_embeddings, gold_embeddings) ])

        bert_score = semantic_similarity_scores # cosine_similarity
        ...
        bse_input = model_output_logits.clone().type(torch.float16).to(self.device)
...
class SemTextual_Logue_Semantic_Loss_Trainer(SFTTrainer):
  ...
  def compute_loss(self, model, inputs, return_outputs=False):
    ...
    semantic_similarity_scores = torch.stack([ torch.tensor( model.cosine_similarity(model_output, gold), dtype=torch.float16 ).to(model.device) for model_output, gold in zip(model_output_embeddings, gold_embeddings) ])
...
# instantiate Sem-textual Logue Semantic Model
sem_text_logue_semantic_model = Sem_Textual_Logue_Semantic_Model()

trainer = SemTextual_Logue_Semantic_Loss_Trainer(
    model = sem_text_logue_semantic_model,
    model_init = sem_text_logue_semantic_model.__init__(),
...
print(f"Total time taken for Fine-tuning Llama2 on SemTextualLogue-Semantic-Loss : {t2 - t1} seconds")
