...
dataset_type = "MULTIWoz_finetune_Semantic_Weighted_CE_Loss_Train_CLM"
SAVE_MODEL_NAME = "llama2-finetuned-Semantic-Weighted-CE-Multiwoz-model"
...
class Weighted_Semantic_CE_M_Loss_Trainer(SFTTrainer):

  def __init__(self, *args, **kwargs):
      super(Weighted_Semantic_CE_M_Loss_Trainer, self).__init__(*args, **kwargs)
  ...
  def compute_loss(self, model, inputs, return_outputs=False):
    ...
    semantic_score_weights = [ 1 - cosine_similarity(model_output, gold) for model_output, gold in zip(model_output_embeddings, gold_embeddings) ]
    L_fn = torch.nn.CrossEntropyLoss(reduction='mean') # default reduction method is set...
    ...
    weighted_semantic_ce_m_loss = torch.stack([semantic_w*ce_l  for ce_l, semantic_w in zip(ce_loss, semantic_score_weights)])
    batched_weighted_semantic_ce_m_loss = torch.mean(weighted_semantic_ce_m_loss)

    return (batched_weighted_semantic_ce_m_loss, outputs) if return_outputs else batched_weighted_semantic_ce_m_loss

t1 = time.time()

trainer = Weighted_Semantic_CE_M_Loss_Trainer(
...
print(f"Total time taken for Fine-tuning Llama2 on Semantic-Weighted CE-Loss : {t2 - t1} seconds")
