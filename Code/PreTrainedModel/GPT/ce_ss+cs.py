...
class datasetClass(Dataset):
    def __init__(self, dataset_path = './multiwoz_2.2.json',variation = 'train'):
        super().__init__()
...
def calculate_loss(logits,work_dataset_tens,source_tens_len,target_tens_len,source_sentence,target_sentence):
    ...
    semantic_bert_similarity = calculate_bert_similarity(generated_sentence,target_sentence)
    context_bert_similarity = calculate_bert_similarity(generated_sentence,source_sentence)

    contanic_score = 0.3*context_bert_similarity+0.7*semantic_bert_similarity
    ...
    total_loss = source_loss+(1-contanic_score)*target_loss
    return total_loss
...
models_folder = "multiwoz_models/ce_ss+cs"
