...
def calculate_sacre_bleu(hyps,refs):
    ...
    dic_result={"Bleu":result,"Blue-1":temp[0],"Blue-2":temp[1],"Blue-3":temp[2],"Blue-4":temp[3]}
    return dic_result
...
def calculate_meteor(hyps, refs):
    ...
    for cur_refs, cur_hyps in zip(refs, hyps):
        total += meteor_score.meteor_score([cur_refs], cur_hyps)
...
def calculate_scores(source_sentences,target_sentences,predicted_sentences):
    results={}
    ...
    results["bert_contanic_score"]=0.7*results["bert_similarity"]+0.3*results["bert_context_similarity"]
